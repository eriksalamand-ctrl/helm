...
["Learning Lab", "M9 2h6v2h-1v4.2l4.6 8A2 2 0 0 1 16.8 19H7.2a2 2 0 0 1-1.8-2.8L10 8.2V4H9V2Zm3 8-2.6 5h5.2L12 10Z"],
  ...
  return (
    <section className="pm-card pm-perf">
      ...
      <div className="pm-chart-area">
        <AreaChart data={data} benchmark={benchData} accent={accent} showBenchmark={benchOn} height={220} />
      </div>
    </section>);
...
function AccountSummary({ accent }) {
  const D = window.PMData;
  const rows = (D.accounts || []).map((a) => {
    const v = D.buildView(a.id).kpis;
    const cash = v.cash || 0,total = v.equity || 0,inv = Math.max(0, total - cash);
    return { id: a.id, name: a.name, label: a.label, cash, inv, total, pl: v.totalPlAbs || 0, plPct: v.totalPlPct || 0 };
  });
  const tot = rows.reduce((s, r) => ({ cash: s.cash + r.cash, inv: s.inv + r.inv, total: s.total + r.total, pl: s.pl + r.pl }), { cash: 0, inv: 0, total: 0, pl: 0 });
  const totPct = tot.total - tot.pl ? tot.pl / (tot.total - tot.pl) * 100 : 0;
  return (
    <section className="pm-card pm-acctsum">
      <div className="pm-card-eyebrow">Accounts · cash, investments & gain</div>
      <div className="pm-table-wrap">
        <table className="pm-table pm-acctsum-table">
          <thead><tr><th className="ta-left">Account</th><th className="ta-right">Cash</th><th className="ta-right">Investments</th><th className="ta-right">Total</th><th className="ta-right">Gain</th></tr></thead>
          <tbody>
            {rows.map((r) =>
            <tr key={r.id}>
                <td className="ta-left"><strong>{r.name}</strong> <span className="pm-acctsum-lbl">{r.label}</span></td>
                ...
                <td className="ta-right mono" style={{ color: sc(r.pl) }}>{fmtPct(r.plPct)}</td>
              </tr>
            )}
            <tr className="pm-acctsum-total">
              ...
              <td className="ta-right mono" style={{ color: sc(tot.pl) }}>{fmtPct(totPct)}</td>
            </tr>
          </tbody>
        </table>
      </div>
...
function Dashboard({ accent }) {
  ...
  if (research) {
    ...
    content = window.LearningLab ? <LearningLab accent={accent} /> : null;
    ...
    content = <LearningPage accent={accent} />;
