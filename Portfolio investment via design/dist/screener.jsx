        ...
        <div className="pm-table-wrap">
          <table className="pm-table scr-table">
            <thead><tr>
              ...
              <th className="ta-center" title="Secular = price vs 200-day average · Weekly = vs 20-day; date = when the current signal started (GMI-style)">Trend sec·wk</th>
            </tr></thead>
            <tbody>
                  ...
                  <React.Fragment key={h.ticker}>
                    <tr className={`scr-row${open ? " open" : ""}`} onClick={() => setOpenTkr(open ? null : h.ticker)}>
                      ...
                      <td className="ta-center">{(() => {
                        const t2 = window.HelmOdds && window.HelmOdds.trend2(h.ticker);
                        if (!t2) return <span className="mono" style={{ color: "var(--muted)" }}>—</span>;
                        const chip = (t, l) => <span style={{ color: t.up ? scUP : scDN }}>{t.up ? "▲" : "▼"} {l} · {t.since}</span>;
                        return <div className="scr-t2">{chip(t2.secular, "sec")}{chip(t2.weekly, "wk")}</div>;
                      })()}</td>
                    </tr>
                    {open && (
                      <tr className="scr-detail-row"><td colSpan={11}>
