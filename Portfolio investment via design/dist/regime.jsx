...
function classifyRegime() {
  ...
  // ---- confirmation-days hysteresis (GMI pattern: "Pending 2/5 days") ----
  // the RAW read can flip on one noisy print; the CONFIRMED regime only flips after
  // the raw read disagrees for 5 distinct days. Stored in localStorage, honest chip in UI.
  const CONF_DAYS = 5, CKEY = "helm_regime_conf_v1";
  const isLive = !!(window.HelmFeed && window.HelmFeed.status && window.HelmFeed.status.live);
  let st = null; try { st = JSON.parse(localStorage.getItem(CKEY) || "null"); } catch (e) {}
  const day = new Date().toISOString().slice(0, 10);
  if (!st || !st.confirmed || !REGIME_GRID[st.confirmed]) st = { confirmed: key, since: day, live: isLive, pending: null, pendingDays: [] };
  // hysteresis only arbitrates between LIVE reads: a store seeded from the demo constant
  // is replaced outright the first time a live classification arrives (no 5-day wait),
  // and demo reads never accrue pending days against a live-confirmed regime.
  if (isLive && !st.live) st = { confirmed: key, since: day, live: true, pending: null, pendingDays: [] };
  else if (!isLive && st.live) { /* demo read vs live store — ignore, keep confirmed */ }
  else if (key === st.confirmed) { st.pending = null; st.pendingDays = []; }
  else {
    if (st.pending !== key) { st.pending = key; st.pendingDays = [day]; }
    else if (!st.pendingDays.includes(day)) st.pendingDays.push(day);
    if (st.pendingDays.length >= CONF_DAYS) { st = { confirmed: key, since: day, live: isLive, pending: null, pendingDays: [] }; }
  }
  try { localStorage.setItem(CKEY, JSON.stringify(st)); } catch (e) {}
  const ckey = st.confirmed;
  const [cGrowth, cInflation] = ckey.split("|");
  const r = REGIME_GRID[ckey] || REGIME_GRID["Stable|Sticky"];
  return { ...r, key: ckey, growth: cGrowth, inflation: cInflation, cpiNow, nlTrend, since: st.since,
    pending: st.pending ? { label: (REGIME_GRID[st.pending] || {}).label || st.pending, have: st.pendingDays.length, need: CONF_DAYS } : null,
    raw: { key, label: (REGIME_GRID[key] || {}).label } };
}
window.HelmRegimeCompute = classifyRegime; // consumers (chief.jsx) can classify without the panel mounted
...
function RegimePanel({ accent }) {
  ...
  useRegEffect(() => {
    window.HelmRegime = { label: r.label, bias: r.bias, key: r.key, geoScore, since: r.since, pending: r.pending };
    ...
    <div className="reg">
      ...
      <section className="pm-card reg-head">
        <div className="reg-head-l">
          ...
          <div className="reg-name" style={{ color: r.col }}>{r.label}
            <span className="reg-since">since {new Date(r.since + "T12:00").toLocaleDateString("en-CA", { month: "short", day: "numeric" })}</span>
            {r.pending && <span className="reg-pending" title={`Raw read disagrees — flips only after ${r.pending.need} consecutive days`}>pending → {r.pending.label} · {r.pending.have}/{r.pending.need}d</span>}
          </div>
...
.reg-since { font-size: 10.5px; font-weight: 600; color: var(--muted); font-family: var(--mono); margin-left: 10px; vertical-align: 3px; }
.reg-pending { font-size: 10.5px; font-weight: 700; color: #b45309; background: #d9770614; border: 1px solid #d9770633; border-radius: 99px; padding: 2px 9px; margin-left: 8px; vertical-align: 3px; white-space: nowrap; }
