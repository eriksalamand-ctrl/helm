...
function BridgeSurface({ accent, onPick }) {
  ...
  useBrEffect(() => { let on = true; window.HelmChief.polishBrief(fp, brief).then((t) => { if (on) setProse(t || null); }); return () => { on = false; }; }, [feedTick]); // t=null resets stale prose → cited facts show
  const { K, regime, props, news, breaches } = fp;
  const [open, setOpen] = useBrState(null); // expanded card ticker
  const [talkOpen, setTalkOpen] = useBrState(true);
  const tt = useBrMemo(() => { try { return window.HelmChief.tableTalk ? window.HelmChief.tableTalk(fp) : null; } catch (e) { return null; } }, [fp]);
  ...
  // ---- printable daily digest: same facts, paper form ----
  function printDigest() {
    const esc = (s) => String(s == null ? "" : s).replace(/&/g, "&amp;").replace(/</g, "&lt;");
    const dt = new Date().toLocaleDateString("en-CA", { weekday: "long", year: "numeric", month: "long", day: "numeric" });
    const kCol = (k) => k === "Buy" ? "#1d9a6c" : k === "Exit" ? "#c43d3d" : "#b45309";
    const briefHtml = prose ? `<p class="lead">${esc(prose)}</p>`
      : `<p class="lead">${brief.sentences.map((s) => `${esc(s.txt)} <span class="src">${esc(s.src)}</span>`).join(" ")}</p>`;
    const pulse = (() => { try { const p = window.HelmPulse && window.HelmPulse.compute(); return p ? `Pulse ${Math.round(p.score)} · ${p.label}` : ""; } catch (e) { return ""; } })();
    const chips = [regime && (regime.label || regime.key) ? `Regime: ${esc(regime.label || regime.key)}` : "", pulse, `Net liq ${brMoney(K.equity)} · ${(K.dayChangePct >= 0 ? "+" : "−") + Math.abs(K.dayChangePct).toFixed(1)}% today`].filter(Boolean);
    const cardHtml = (c) => `
      <div class="card">
        <div class="ct"><b class="act" style="color:${kCol(c.kind)}">${c.kind.toUpperCase()}</b> <b>${esc(c.ticker)}</b> <span class="nm">${esc(c.name)}${c.held ? "" : " · new"}</span>
          <span class="sz">${c.kind === "Buy" && c.route ? `${brMoney(c.route.amt)} · ${c.shares} sh · ${esc(c.route.acctName)}` : c.kind === "Trim" ? "sell ~⅓, keep the rest" : "close position"} · SL ${brMoney(c.sig.stop)} · TP ${brMoney(c.sig.target)}</span></div>
        <div class="th">${esc(c.thesis)}</div>
        ${c.ev.map((e) => `<div class="ev"><span class="tag">${esc(e.tag)}</span> ${esc(e.txt)}</div>`).join("")}
        <div class="st">${["vera", "iris", "flint"].map((k) => `<span><b>${BR_CREW[k].n}</b> ${esc(c.stances[k][0])} — ${esc(c.stances[k][1])}</span>`).join("")}
          <span><b>Gates</b> ${c.gates.ok ? "✓" : "✗"}${c.gates.notes.length ? " — " + esc(c.gates.notes[0]) : ""}</span>
          ${c.vote ? `<span><b>Table</b> ${esc(c.vote.verdict)}${c.vote.sizeFactor < 1 ? ` · size ×${c.vote.sizeFactor}` : ""}</span>` : ""}</div>
        <div class="kill"><b>Kill:</b> ${esc(c.kill)}</div>
      </div>`;
    const w = window.open("", "_blank", "width=880,height=980");
    if (!w) { alert("Pop-up blocked — allow pop-ups to print the digest."); return; }
    w.document.write(`<!DOCTYPE html><html><head><meta charset="utf-8"><title>Helm — Chief's digest · ${dt}</title><style>
      @page{size:letter;margin:0.7in}body{font:13px/1.45 "Helvetica Neue",Helvetica,Arial,sans-serif;color:#16181d;margin:0;padding:28px;max-width:760px}
      h1{font-size:19px;margin:0}
      .meta{color:#6b7280;font-size:12px;margin:2px 0 14px}
      .chips{display:flex;gap:8px;flex-wrap:wrap;margin:0 0 14px}.chips span{border:1px solid #d7dbe0;border-radius:99px;padding:3px 10px;font-size:11.5px;font-weight:600}
      .lead{font-size:13.5px;margin:0 0 6px}.src{font-size:9.5px;color:#8a919c;text-transform:uppercase;letter-spacing:.04em}
      .do{font-weight:700;border-top:1px solid #e5e7eb;border-bottom:1px solid #e5e7eb;padding:8px 0;margin:10px 0 16px}
      .talk{font-size:12px;margin:4px 0;color:#374151}.talk b{color:#16181d}
      h2{font-size:11px;text-transform:uppercase;letter-spacing:.07em;color:#6b7280;margin:18px 0 8px}
      .card{border:1px solid #e2e5e9;border-radius:8px;padding:10px 12px;margin-bottom:10px;page-break-inside:avoid}
      .ct{display:flex;gap:8px;align-items:baseline;flex-wrap:wrap}.ct .nm{color:#6b7280}.ct .sz{margin-left:auto;font-size:11.5px;color:#374151}
      .th{margin:6px 0;font-size:12.5px}
      .ev{font-size:11.5px;color:#374151;margin:2px 0}.tag{display:inline-block;border:1px solid #d7dbe0;border-radius:4px;padding:0 5px;font-size:10px;font-weight:700;margin-right:4px}
      .st{display:flex;flex-direction:column;gap:2px;font-size:11px;color:#4b5563;margin-top:6px}.st b{color:#16181d}
      .kill{font-size:11px;color:#8a2424;margin-top:6px}
      .news div,.br div{font-size:12px;margin:3px 0}
      .foot{margin-top:22px;padding-top:10px;border-top:1px solid #e5e7eb;font-size:10.5px;color:#8a919c}
      @media print{.noprint{display:none}}
    </style></head><body>
      <h1>The Chief — daily digest</h1><div class="meta">Helm · ${dt}</div>
      <div class="chips">${chips.map((c) => `<span>${c}</span>`).join("")}</div>
      ${briefHtml}
      <p class="do">${esc(brief.doLine)}</p>
      ${tt ? `<h2>Table talk — why today's call</h2>${[["Iris", tt.iris], ["Vera", tt.vera], ["Flint", tt.flint], ["The Chief", tt.verdict]].map(([n, ls]) => ls.length ? `<div class="talk"><b>${n}</b> ${ls.map((s) => esc(s.txt)).join(" ")}</div>` : "").join("")}` : ""}
      <h2>Round table · ${props.cards.length || "no"} proposition${props.cards.length === 1 ? "" : "s"}</h2>
      ${props.cards.length ? props.cards.map(cardHtml).join("") : `<div class="card">No proposition today — nothing clears the conviction floor at a sane σ-entry. The crew keeps watch.${props.gateLog.length ? "<br>Blocked: " + esc(props.gateLog.map((g) => `${g.t} (${g.g})`).join(" · ")) : ""}</div>`}
      ${breaches.length ? `<h2>Policy watch</h2><div class="br">${breaches.map((b) => `<div><b>${esc(b.t)}</b> — ${esc(b.d)}</div>`).join("")}</div>` : ""}
      ${news.items.length ? `<h2>Material news</h2><div class="news">${news.items.map((n) => `<div>• ${esc(n.headline)}${n.touch && n.touch.length ? ` <b>(${n.touch.join(", ")})</b>` : ""}</div>`).join("")}</div>` : ""}
      <div class="foot">Advisory only — Helm places no orders. Every number derives from the live fact pack (${news.live ? "live feed" : "demo data"}); the table's weights come from measured track records. Generated ${new Date().toLocaleTimeString("en-CA", { hour: "2-digit", minute: "2-digit" })}.</div>
      <script>window.onload = () => setTimeout(() => window.print(), 250);<\/script>
    </body></html>`);
    w.document.close();
  }

  // ---- digest as plain text (for email/notes) ----
  const [copied, setCopied] = useBrState(false);
  function digestText() {
    const dt = new Date().toLocaleDateString("en-CA", { weekday: "long", year: "numeric", month: "long", day: "numeric" });
    const pulse = (() => { try { const p = window.HelmPulse && window.HelmPulse.compute(); return p ? `Pulse ${Math.round(p.score)} ${p.label}` : ""; } catch (e) { return ""; } })();
    const L = [];
    L.push(`HELM — THE CHIEF'S DAILY DIGEST · ${dt}`);
    L.push([regime && (regime.label || regime.key) ? `Regime: ${regime.label || regime.key}` : "", pulse, `Net liq ${brMoney(K.equity)} (${(K.dayChangePct >= 0 ? "+" : "−") + Math.abs(K.dayChangePct).toFixed(1)}% today)`].filter(Boolean).join(" · "));
    L.push("");
    L.push(prose || brief.sentences.map((s) => s.txt).join(" "));
    L.push(`DO: ${brief.doLine}`);
    if (tt) { L.push("", "WHY — TABLE TALK");
      [["Iris", tt.iris], ["Vera", tt.vera], ["Flint", tt.flint], ["The Chief", tt.verdict]].forEach(([n, ls]) => { if (ls.length) L.push(`${n}: ${ls.map((s) => s.txt).join(" ")}`); }); }
    L.push("", `PROPOSITIONS (${props.cards.length || "none"})`);
    if (props.cards.length) props.cards.forEach((c) => {
      L.push(`• ${c.kind.toUpperCase()} ${c.ticker} — ${c.kind === "Buy" && c.route ? `${brMoney(c.route.amt)}, ${c.shares} sh, ${c.route.acctName}` : c.kind === "Trim" ? "sell ~1/3, keep the rest" : "close position"} · SL ${brMoney(c.sig.stop)} / TP ${brMoney(c.sig.target)}${c.vote ? ` · table: ${c.vote.verdict}${c.vote.sizeFactor < 1 ? ` (size ×${c.vote.sizeFactor})` : ""}` : ""}`);
      L.push(`  ${c.thesis}`);
      L.push(`  Kill: ${c.kill}`);
    });
    else L.push(`None clears the conviction floor at a sane σ-entry.${props.gateLog.length ? " Blocked: " + props.gateLog.map((g) => `${g.t} (${g.g})`).join(", ") : ""}`);
    if (breaches.length) { L.push("", "POLICY WATCH"); breaches.forEach((b) => L.push(`• ${b.t} — ${b.d}`)); }
    if (news.items.length) { L.push("", "MATERIAL NEWS"); news.items.forEach((n) => L.push(`• ${n.headline}${n.touch && n.touch.length ? ` (${n.touch.join(", ")})` : ""}`)); }
    L.push("", `Advisory only — Helm places no orders. ${news.live ? "Live feed" : "Demo data"}.`);
    return L.join("\n");
  }
  function copyDigest() {
    const t = digestText();
  ...
  function fallbackCopy(t, done) {
    ...
    try { document.execCommand("copy"); done(); } catch (e) { alert("Copy failed — select the digest in the print view instead."); }
    ...
    <div className="br-grid">
      <div className="br-main">
        ...
        <section className="pm-card br-brief">
          <div className="br-brief-head">
            ...
            <div>
              <div className="br-eyebrow">The Chief — morning brief</div>
            ...
            <button className="br-print" title="Copy the digest as plain text (email, notes)" onClick={copyDigest}>{copied ? "✓ copied" : "⧉ copy"}</button>
            <button className="br-print" title="Print / save the digest as PDF" onClick={printDigest}>⎙ digest</button>
          </div>
          {prose ? (
          ...
          {tt && (
            <div className="br-talk">
              <button className="br-talk-btn" onClick={() => setTalkOpen(!talkOpen)}>{talkOpen ? "▾" : "▸"} Table talk — why today's call</button>
              {talkOpen && (
                <div className="br-talk-body">
                  {[["iris", tt.iris], ["vera", tt.vera], ["flint", tt.flint]].map(([k, lines]) => lines.length > 0 && (
                    <div className="br-talk-row" key={k}>
                      <Ava a={BR_CREW[k]} size={20} />
                      <div>{lines.map((s, i) => <span key={i}>{s.txt} <span className="br-src">{s.src}</span>{" "}</span>)}</div>
                    </div>
                  ))}
                  <div className="br-talk-row br-talk-chief">
                    <Ava a={BR_CREW.rhodes} size={20} />
                    <div>{tt.verdict.map((s, i) => <span key={i}><b>{s.txt}</b>{" "}</span>)}</div>
                  </div>
                </div>
              )}
            </div>
      ...
      <div className="br-rail">
        ...
        <section className="pm-card">
          ...
          {window.LedgerBoard ? <window.LedgerBoard /> : (() => {
...
.br-talk { margin-top: 4px; border-top: 1px solid var(--line-2, #f0f2f5); padding-top: 8px; }
.br-talk-btn { font: inherit; font-size: 11.5px; font-weight: 700; color: var(--muted); background: none; border: none; padding: 0; cursor: pointer; text-transform: uppercase; letter-spacing: 0.05em; }
.br-talk-btn:hover { color: var(--accent); }
.br-talk-body { display: flex; flex-direction: column; gap: 7px; margin-top: 9px; }
.br-talk-row { display: flex; gap: 8px; font-size: 12.5px; line-height: 1.55; color: var(--ink-2); }
.br-talk-row .br-ava { margin-top: 2px; }
.br-talk-chief { color: var(--ink); }
.br-print { font: inherit; font-size: 11.5px; font-weight: 600; color: var(--muted); background: none; border: 1px solid var(--line); border-radius: 7px; padding: 4px 9px; cursor: pointer; flex: none; align-self: flex-start; }
.br-print:hover { color: var(--accent); border-color: var(--accent); }
