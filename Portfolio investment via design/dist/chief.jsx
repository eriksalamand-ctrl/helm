...
(function () {
  ...
  function buildCards() {
    ...
    const regime = window.HelmRegime || {};
    const news = getNews();
    ...
    const mkCard = (x, kind) => {
      ...
      const stances = {
        vera: s.realFund ? (s.qualityScore >= 55 ? ["BUY", "quality real & sound"] : ["CAUTION", "quality " + s.qualityScore]) : ["WATCH", "proxy data"],
      ...
      return { kind, ticker: h.ticker, name: h.name, held: !!h.held, sig: s, e, edge: x.edge,
        ...
        ev, kill, stances, gates, route, shares, dissent };
    };
  ...
  function factPack() {
    ...
    const view = D().buildView("all");
    const K = view.kpis;
    let regime = window.HelmRegime || null;
    if (!regime && window.classifyRegime) {
      try { const r = window.classifyRegime(); regime = { label: r.label, bias: r.bias, key: r.key }; window.HelmRegime = regime; } catch (e) {}
    }
