...
(function () {
  ...
  function factPack() {
    ...
    try { window.HelmLedger && window.HelmLedger.stamp(props.cards, regime); } catch (e) {} // Quill's book: every proposition is journaled + scored
    const news = getNews();
    const breaches = [];
  ...
  // ---- table talk: what each voice actually said today — the WHY behind cards / no cards ----
  function tableTalk(fp) {
    const { regime, props, news, breaches } = fp;
    const iris = [], vera = [], flint = [], verdict = [];
    // Iris — macro backdrop + geopolitics that reaches the book or the indices
    if (regime) iris.push({ txt: `Backdrop is ${regime.label || regime.key}${regime.since ? ` (confirmed since ${regime.since})` : ""}${regime.pending ? ` — pending flip to ${regime.pending}` : ""}.`, src: "REGIME" });
    try { const p = window.HelmPulse && window.HelmPulse.compute();
      if (p) iris.push({ txt: `Pulse ${Math.round(p.score)} — ${p.label}${p.stress && p.stress.length ? `; stress in ${p.stress.join(", ")}` : ""}.`, src: "PULSE" }); } catch (e) {}
    try { const tx = window.HelmTransmission && window.HelmTransmission.alerts();
      const hot = ((tx && tx.list) || []).filter((a) => a.it && a.it.label === "hot");
      if (hot.length) iris.push({ txt: `Geopolitics: ${hot.map((a) => `${a.name} (${a.vol ? "volatility node" : "−$" + Math.round((a.hurt || 0) / 1000) + "k at risk"})`).join("; ")} — details in Transmission.`, src: "TAPE" });
      else iris.push({ txt: "No chokepoint runs hot on today's tape — geopolitics is background noise for the book.", src: "TAPE" }); } catch (e) {}
    const macroNews = news.items.filter((n) => n.macro);
    if (macroNews.length) iris.push({ txt: `Macro tape: ${macroNews.slice(0, 2).map((n) => `“${n.headline}”`).join(" · ")}.`, src: "NEWS" });
    // Vera — name-level: news touching holdings + open intake claims
    const touched = news.items.filter((n) => n.touch && n.touch.length);
    const touchedT = [...new Set(touched.flatMap((n) => n.touch))];
    vera.push(touched.length
      ? { txt: `Holdings in the news: ${touchedT.join(", ")} — read before acting; headlines are half the story.`, src: "NEWS" }
      : { txt: "No held name is in today's tape — clean slate at name level.", src: "NEWS" });
    try { const I = window.HelmIntake, J = I && I.load ? I.load() : null;
      if (J && J.claims && J.claims.length) { const open = J.claims.map(I.scoreClaim).filter((c) => c.status === "open");
        if (open.length) { const b = open.filter((c) => c.direction === "bullish").length;
          vera.push({ txt: `Tracking ${open.length} open intake claim${open.length > 1 ? "s" : ""} (${b} bullish / ${open.length - b} bearish) — sources earn weight only as they resolve.`, src: "INTAKE" }); } } } catch (e) {}
    // Flint — what the scan produced and what the gates ate
    flint.push({ txt: `Scanned ${props.scanned} names against floor ${props.floor}; ${props.cards.length || "none"} made the table.`, src: "SCAN" });
    if (props.gateLog.length) { const g = {}; props.gateLog.forEach((x) => { const k = x.g.replace(/^G\d+\s*/, ""); (g[k] = g[k] || []).push(x.t); });
      flint.push({ txt: `Gates blocked ${props.gateLog.length}: ${Object.entries(g).map(([k, ts]) => `${ts.join("/")} (${k})`).join("; ")}.`, src: "GATES" }); }
    // The Chief — why today's call is what it is
    verdict.push({ txt: props.cards.length
      ? `Cards stand because conviction cleared the floor WITH real data at a sane σ-entry${macroNews.length || breaches.length ? ", despite the noise above" : " and the backdrop doesn't argue"} — size per the table's verdict.`
      : `No recommendation is the decision: ${props.gateLog.length ? "everything that scored high enough failed a gate (σ-chase, knife, or data quality)" : "nothing scored past the conviction floor"}${breaches.length ? ", and " + breaches[0].t.toLowerCase() + " needs fixing first" : ""}. Forcing a trade here is how edges die.`, src: "CHIEF" });
    return { iris, vera, flint, verdict };
  }

  window.HelmChief = { factPack, buildCards, getNews, composeBrief, polishBrief, tableTalk };
})();
